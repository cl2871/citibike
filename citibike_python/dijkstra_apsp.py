...
def dijkstra_min_heap(vertices, source_distances, edges, prev, source):

    """Implementation of dijkstra using a min heap as a priority queue.

    Priority queue utilized has distance from source used for priority (lower distance is higher in priority).
    Updates the source_distances and prev dicts.
    ...
    for vertex in vertices:
        if vertex != source:
            source_distances[vertex] = float("inf")             # initialize distance from source as infinity
            prev[vertex] = None                                 # initialize next node from vertex to source as null
